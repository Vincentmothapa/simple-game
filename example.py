...
monster_height = enemy2.get_height()
monster_width = enemy2.get_width()

player_height = enemy3.get_height()
player_width = enemy3.get_width()
...
while 1: # This is a looping structure that will loop the indented code until you tell it to stop(in the event where you exit the program by quitting). In Python the int 1 has the boolean value of 'true'. In fact numbers greater than 0 also do. 0 on the other hand has a boolean value of false. You can test this out with the bool(...) function to see what boolean value types have. You will learn more about while loop structers later. 
    ...
    screen.blit(player, (playerXPosition, playerYPosition))# This draws the player image to the screen at the postion specified. I.e. (100, 50).
    ...
    if keyLeft == True:
        if playerXPosition > 0 : # This makes sure that the user does not move the player beyong the window.
            playerXPosition -= 1
    if keyRight == True:
        if playerXPosition < screen_width - image_width:# This makes sure that the user does not move the player beyond the window.
            playerXPosition += 1
...
# PRIZE
    if playerBox.colliderect(prizeBox):
        print("You win a special prize! Congratulations")
...
#=================================================================
    ...
    # Make enemies and prize approach the player at varrying speeds.
    enemyXPosition -= 0.1
    enemy2XPosition -= 0.2
    enemy3XPosition -= 0.3
    prizeXPosition -= 0.2
